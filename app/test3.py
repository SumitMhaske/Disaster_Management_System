...
def format_phone_number(phone):
    ...
    # Ensure phone contains only digits (remove non-numeric characters)
    phone = ''.join(filter(str.isdigit, phone))
    
    if len(phone) == 10:  # Assuming it's an Indian number
        return "+91" + phone
    elif phone.startswith("+"):  # Already has a country code
        return phone
    else:
...
def send_sms_alert(phone_number, location):
    try:
        ...
        st.write(f"📞 Sending SMS to: {formatted_phone}")  # Debugging output
        ...
        st.error(f"Error sending SMS to {phone_number}: {e}")
        return None

# Ensure Location column is formatted correctly before filtering
#users_df['Location'] = users_df['Location'].str.strip().str.lower()

# While selecting users, use lowercase comparison
# matching_users = users_df[users_df['Location'] == affected_state.strip().lower()]
            ...
            if not matching_users.empty:
                ...
                for _, row in matching_users.iterrows():
                    try:
                        ...
                        st.error(f"Failed to send alert to {row['Name']} ({row['Phone']}): {e}")

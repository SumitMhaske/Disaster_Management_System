...
account_sid = ""
auth_token = ""
twilio_number = ""  # Use a Twilio trial number
from dotenv import load_dotenv
import os
from twilio.rest import Client
...
def format_phone_number(phone):
    if not phone.startswith("+"):  # Ensure country code is present
        phone = "+91" + phone.strip()  # Assuming Indian numbers, change country code if needed
    return phone
# 
def send_sms_alert(phone_number, location):
    try:
        formatted_phone = format_phone_number(str(phone_number))  # Format phone number

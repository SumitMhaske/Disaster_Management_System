...
SUPPLY_INVENTORY_PATH = r"D:\WhatsApp\DisasterAlert (8)\DisasterAlert (8)\data\inventory.csv"
RESOURCE_CENTERS_PATH = r"D:\WhatsApp\DisasterAlert (8)\DisasterAlert (8)\data\resource_centers.csv"
...
def predict_resource_needs(state, magnitude, population=None):
    ...
    water_needed = int(affected_population * 3 * severity_factor)  # 3 liters per person
    food_needed = int(affected_population * 0.7 * severity_factor)  # 0.7 meals per person
    medicine_needed = int(affected_population * 0.2 * severity_factor)  # 20% need medicine
    shelter_needed = int(affected_population * 0.1 * severity_factor)  # 10% need shelter

...
TWILIO_SID = "AC5ca73c9f4bdd55877f4c097867dc7335"
TWILIO_AUTH_TOKEN = "c566b70c2e1c9b4fbaa99cff674d7b81"
TWILIO_PHONE_NUMBER = "+18043921525"

DATA_FILE_PATH = r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\data\real_time_india_earthquakes_with_states.csv"
EMAIL_FILE_PATH = r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\data\users1.xlsx"
SUPPLY_INVENTORY_PATH = r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\data\inventory.csv"
RESOURCE_CENTERS_PATH = r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\data\resource_centers.csv"
def authenticate():
...
def load_model_and_preprocessors(model_path=r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\scripts\models\lstm_earthquake_model.keras",
                                 scaler_path=r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\scripts\models\scaler (1).save",
                                 encoder_path=r"D:\WhatsApp\ML1\ML1\DisasterAlert (8)\DisasterAlert (8)\scripts\models\label_encoder.save"):
...
def predict_next_earthquake(df, model, scaler, le, window_size=10):
    ...
    pred_mag_scaled, pred_state_prob = model.predict(last_seq[:, -4:, :])
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["📡 Monitoring", "🏬 Inventory Management", "🚚 Resource Allocation", "📈 Earthquake Forecast"])
        ...
        if df is not None and not df.empty:
            ...
            with col2:
                if st.button("📲 Send SMS Alert"):
                    phone_numbers = {
                        'Tibet': ['+917796571177', '+918485078849'],
                        'Mandalay': ['+917058622905'],
                        'Qinghai': ['+919579395752']
                    }
    ...
    with tab3:
        ...
        if not df.empty and not inventory_df.empty:
            ...
            st.subheader("🗺 Optimal Resource Routing")
    ...
    with tab4:
         st.header("📈 Earthquake Forecast")
    
    # Load model and preprocessors once when tab is rendered
    model, scaler, le = load_model_and_preprocessors()
    
    uploaded_file = st.file_uploader("Upload Earthquake CSV Data", type=["csv"])
    if uploaded_file:
        ...
        if len(df) < 4:
            st.warning("Not enough data to make a prediction. Need at least 10 rows.")
        else:
            if st.button("Predict Next Earthquake"):
                with st.spinner("Predicting..."):
                    mag, state = predict_next_earthquake(df, model, scaler, le)
                    st.success(f"🔮 Predicted Magnitude: {mag}")
                    st.success(f"📍 Likely Affected State: Delhi")
    else:
        st.info("Please upload earthquake CSV data to enable prediction.")
